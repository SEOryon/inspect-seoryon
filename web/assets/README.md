...
The landing page references the real product screenshots below. Until they
exist, each `<img>` falls back to a clearly-labeled placeholder (wired in
`app.js`), so the page never shows a broken image.
...
square. Frames already apply the rounded corner + border + soft shadow.

> TODO: drop real screenshots.
